(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const isMetricsLocked = () => window.BKAdminMetricsLock === true;
    const isStoresLocked = () => window.BKAdminStoresLock === true;
    ...
    const normalizeApprovalType = (value) => {
      ...
      if (raw === "task-post" || raw === "task" || raw === "mission") return "task";
    ...
    const mapApprovalRow = (row) => {
      ...
      return {
        approvalId: row.id || row.approvalId || "",
        type: normalizeApprovalType(row.type) || row.type || "",
        status: String(row.status || "").toLowerCase(),
        reason: row.reason || "",
        payload: row.payload || row.detail || {},
        user: row.user || {},
        submittedAt: row.createdAt || row.created_at || row.submittedAt || row.submitted_at || "",
    ...
    const fetchApprovalList = async () => {
      ...
      if (approvalEtag) headers["If-None-Match"] = approvalEtag;
      const response = await fetch("/api/admin/approvals?perPage=200", { headers, cache: "no-store" });
      if (response.status === 304 && approvalCache.length) return approvalCache.slice();
      const data = await response.json().catch(() => null);
      if (!response.ok || !data || data.ok === false) {
        const error = data && data.error ? data.error : "REQUEST_FAILED";
        throw new Error(error);
      }
      const items = (data.items || []).map(mapApprovalRow).filter(Boolean);
      approvalCache = items.slice();
      const nextEtag = response.headers.get("etag") || response.headers.get("ETag") || "";
      if (nextEtag) approvalEtag = nextEtag;
      return items;
    };

    const updateApprovalStatus = async (approvalId, status, reason) => {
      if (!approvalId) throw new Error("INVALID_REQUEST");
      const headers = getAdminHeaders() || {};
      if (!headers || !headers["x-admin-user"] || !headers["x-admin-pass"]) {
        throw new Error("UNAUTHORIZED");
      }
      const response = await fetch(`/api/admin/approvals/${encodeURIComponent(approvalId)}`, {
        method: "POST",
      ...
      const index = approvalState.data.findIndex((item) => item && item.approvalId === approvalId);
      if (index >= 0) {
        const current = approvalState.data[index];
        approvalState.data[index] = {
          ...current,
          status: normalizedStatus || current.status,
          reason: normalizedStatus === "rejected" ? reason || current.reason || "" : "",
          reviewedAt: new Date().toISOString(),
        };
        approvalCache = approvalState.data.slice();
      }
      approvalEtag = "";
      return data;
    };

    services.approvals.list = fetchApprovalList;
    services.approvals.updateStatus = updateApprovalStatus;
    ...
    const buildChartPath = (points, width, height, padding) => {
      ...
      const rangeMin = max === min ? min - 1 : min;
      const rangeMax = max === min ? max + 1 : max;
      const span = rangeMax - rangeMin || 1;
      const usableHeight = height - padding * 2;
      const coords = points.map((value, index) => {
        const x = points.length === 1 ? 0 : (index / (points.length - 1)) * width;
        const y = padding + (1 - (value - rangeMin) / span) * usableHeight;
        return { x, y };
      });
    ...
    const renderApprovals = () => {
      ...
      const typeFilter = normalizeApprovalType(approvalState.type);
      const statusFilter = String(approvalState.status || "all").toLowerCase();
      const search = normalizeText(approvalState.search || "");
      const list = (Array.isArray(approvalState.data) ? approvalState.data : [])
        .filter((item) => {
          if (!item) return false;
          const itemType = normalizeApprovalType(item.type) || item.type || "";
          ...
          const payload = item.payload || item.detail || {};
          const haystack = [
            user.displayName,
            user.username,
            user.email,
        ...
        .sort((a, b) => {
          if (approvalState.sort === "user") {
            const aUser = a.user || {};
            const bUser = b.user || {};
            return normalizeText(aUser.displayName || aUser.username || aUser.email || "").localeCompare(
              normalizeText(bUser.displayName || bUser.username || bUser.email || "")
            );
          }
          return parseDateValue(b.submittedAt || b.createdAt) - parseDateValue(a.submittedAt || a.createdAt);
        });
      if (!list.length) {
      ...
      approvalsBody.innerHTML = pageItems
        .map((item) => {
          const user = item.user || {};
          const payload = item.payload || item.detail || {};
          const typeValue = normalizeApprovalType(item.type) || item.type || "";
          const typeLabel = typeValue === "seller" ? "Seller mới" : typeValue === "task" ? "Quyền đăng bài" : "Khác";
          const statusMap = {
          ...
          const statusMeta = statusMap[statusValue] || { label: statusValue || "--", className: "warn" };

          const infoParts = [];
          const extraParts = [];
          if (typeValue === "seller") {
            const fullName = payload.fullName || payload.full_name || "";
            const birth = payload.birthDate || payload.birth || "";
            const phone = payload.phone || "";
            const country = payload.country || "";
            const address = payload.address || "";
            const reason = payload.reason || payload.note || "";
            if (fullName) infoParts.push(fullName);
            if (birth) infoParts.push(birth);
            if (phone) infoParts.push(phone);
            if (country) infoParts.push(country);
            if (address) extraParts.push(`Địa chỉ: ${address}`);
            if (reason) extraParts.push(reason);
          } else {
            const contact = payload.contact || "";
            const budgetRaw = payload.budget != null ? payload.budget : payload.budgetVnd;
            const budgetNum = Number(budgetRaw);
            const budget = Number.isFinite(budgetNum) ? formatVnd(budgetNum) : budgetRaw || "";
            const desc = payload.desc || payload.description || payload.reason || "";
            if (contact) infoParts.push(contact);
            if (budget) infoParts.push(`Ngân sách: ${budget}`);
            if (desc) extraParts.push(desc);
          }
          const infoText = infoParts.length ? infoParts.join(" • ") : "--";
          const extraText = extraParts.length ? `<div class="form-hint">${escapeHtml(extraParts.join(" • "))}</div>` : "";
          const reasonText =
            statusValue === "rejected" && item.reason
              ? `<div class="form-hint">Lý do: ${escapeHtml(item.reason)}</div>`
              : "";
          const userLabel = user.displayName || user.username || user.email || "--";
          const userSub = user.username || user.email || user.id || "";
          const actions =
            statusValue === "pending"
              ? `
          ...
          return `
            <tr data-approval-id="${escapeHtml(item.approvalId || "")}">
              <td><strong>${escapeHtml(userLabel)}</strong><div class="form-hint">${escapeHtml(userSub)}</div></td>
              <td>${escapeHtml(typeLabel)}</td>
              <td>${escapeHtml(infoText)}${extraText}</td>
              <td><span class="seller-tag ${statusMeta.className}">${escapeHtml(statusMeta.label)}</span>${reasonText}</td>
              <td>${formatDateLabel(item.submittedAt || item.createdAt)}</td>
              <td class="admin-action-row actions-inline">${actions}</td>
            </tr>
          `;
        })
        .join("");
    ...
    const updateOverview = () => {
      ...
      const metricsLocked = isMetricsLocked();
      ...
      approvalsBody.addEventListener("click", (event) => {
        ...
        if (action === "approve-approval") {
          openModal({
            ...
            onConfirm: async () => {
              const row = approvalsBody.querySelector(`tr[data-approval-id="${approvalId}"]`);
              if (row) row.querySelectorAll("button").forEach((el) => (el.disabled = true));
              try {
                await services.approvals.updateStatus(approvalId, "approved", "");
                renderApprovals();
                updateOverview();
                showToast("Da duyet yeu cau.");
              } catch (error) {
                showToast("Khong the duyet yeu cau.");
              } finally {
                if (row) row.querySelectorAll("button").forEach((el) => (el.disabled = false));
              }
            },
          });
        }
        if (action === "reject-approval") {
          const defaultReason = "Thông tin không hợp lệ";
          openModal({
            ...
            onConfirm: async (note) => {
              ...
              if (row) row.querySelectorAll("button").forEach((el) => (el.disabled = true));
              try {
                await services.approvals.updateStatus(approvalId, "rejected", reason);
                renderApprovals();
                updateOverview();
                showToast("Da tu choi yeu cau.");
              } catch (error) {
                showToast("Khong the tu choi yeu cau.");
              } finally {
                if (row) row.querySelectorAll("button").forEach((el) => (el.disabled = false));
              }
            },
