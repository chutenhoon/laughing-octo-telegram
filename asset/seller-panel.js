(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const storeState = { data: [], loading: true, error: false, page: 1, perPage: 4, search: "", status: "all", sort: "recent" };
    ...
    if (storeCreateBtn) {
      storeCreateBtn.addEventListener("click", () => openStoreEditor(null));
    }
    if (storeCancelBtn) {
    ...
    if (storeGrid) {
      storeGrid.addEventListener("click", (event) => {
        ...
        if (action === "toggle-store" && storeId) {
          openModal({
            title: store && store.active ? "Đóng gian hàng?" : "Mở gian hàng?",
            message: store && store.active ? "Gian hàng sẽ tạm ngừng hiển thị." : "Gian hàng sẽ hiển thị trở lại.",
            onConfirm: () => {
              services.stores.toggleActive(storeId);
              showToast(store && store.active ? "Đã tạm đóng gian hàng." : "Đã mở gian hàng.");
            },
          });
        }
      });
    ...
    loadList(storeState, services.stores.list, () => {
      renderStores();
      updateStoreOptions();
    });
