(function () {
  ...
  window.BKAdminStoresLock = true;
  window.BKAdminMetricsLock = true;
  ...
  const fetchStoreList = async (status, etag, cache) => {
    const headers = buildHeaders();
    if (etag) headers["If-None-Match"] = etag;
    const response = await fetch(`/api/admin/shops?status=${encodeURIComponent(status)}&perPage=100`, {
      headers,
      cache: "no-store",
    });
    if (response.status === 304 && Array.isArray(cache)) {
      return { items: cache, etag };
    }
    const data = await response.json().catch(() => null);
    if (!response.ok || !data || data.ok === false) {
      const error = (data && data.error) || "REQUEST_FAILED";
    ...
    return { items: data.items || [], etag: nextEtag };
  };
  ...
  const loadStores = async (options = {}) => {
    ...
    try {
      const [pending, updates] = await Promise.all([
        fetchStoreList("pending", state.approveEtag, state.approve.data),
        fetchStoreList("pending_update", state.updateEtag, state.update.data),
      ]);
      state.approve.data = pending.items || [];
      state.update.data = updates.items || [];
      if (pending.etag) state.approveEtag = pending.etag;
      if (updates.etag) state.updateEtag = updates.etag;
