(function () {
  ...
  const validateFile = (file) => {
    ...
    if (file.size > SELLER_MAX_SIZE) return "FILE_TOO_LARGE";
    return "";
  };
  ...
  const createManager = (config) => {
    ...
    const render = () => {
      ...
      list.innerHTML = state.items
        ...
        .join("");
      setCount();
      if (upload) upload.disabled = !state.shopId || state.items.length >= SELLER_LIMIT;
    };
    ...
    const uploadFiles = async (files) => {
      ...
      if (state.items.length + fileList.length > SELLER_LIMIT) {
