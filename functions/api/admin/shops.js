...
import { requireAdmin, createSignedMediaToken, buildMediaUrl, computeEtag } from "../_catalog.js";
...
export async function onRequestGet(context) {
  ...
  const etagSeed = rawRows
    ...
    .join(";");
  const etag = `"${await computeEtag(`${status || "all"}:${search || ""}:${page}:${perPage}:${etagSeed}`)}"`;
