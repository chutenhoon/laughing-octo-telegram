...
const COLUMN_DEFS = {
  ...
  shop_images: [
    ...
    { name: "position", def: "INTEGER DEFAULT 1" },
  ...
  order_items: [
    { name: "shop_id", def: "TEXT" },
    { name: "content_text", def: "TEXT" },
    { name: "content_r2_key", def: "TEXT" },
    { name: "content_r2_etag", def: "TEXT" },
  ],
  service_requests: [
...
async function hasUniqueIndexOnColumn(db, table, column) {
  const indexes = await getIndexList(db, table);
  for (const index of indexes) {
    if (!index || !index.name || Number(index.unique || 0) !== 1) continue;
    try {
      const info = await db.prepare(`PRAGMA index_info('${index.name}')`).all();
      const cols = info && Array.isArray(info.results) ? info.results.map((row) => String(row.name || "")) : [];
...
async function rebuildShopsTable(db, report) {
  if (!(await tableExists(db, "shops"))) return false;
  const hasUniqueUser = await hasUniqueIndexOnColumn(db, "shops", "user_id");
  if (!hasUniqueUser) return false;

  const sourceCols = await getColumns(db, "shops");
  const tempTable = "shops_new";
  const newSql = TABLE_DEFS.shops.replace("CREATE TABLE IF NOT EXISTS shops", `CREATE TABLE IF NOT EXISTS ${tempTable}`);

  try {
    await db.prepare("PRAGMA foreign_keys = OFF").run();
    await db.prepare(`DROP TABLE IF EXISTS ${tempTable}`).run();
    await db.prepare(newSql).run();
    const targetCols = await getColumns(db, tempTable);
    const copyCols = Array.from(targetCols).filter((col) => sourceCols.has(col));
    if (copyCols.length) {
      const colList = copyCols.join(", ");
      await db.prepare(`INSERT INTO ${tempTable} (${colList}) SELECT ${colList} FROM shops`).run();
    }
    await db.prepare("DROP TABLE shops").run();
    await db.prepare(`ALTER TABLE ${tempTable} RENAME TO shops`).run();
    await db.prepare("PRAGMA foreign_keys = ON").run();
    report.rebuiltTables = report.rebuiltTables || [];
    report.rebuiltTables.push("shops");
    return true;
  } catch (error) {
    report.errors.push({ table: "shops", action: "rebuild", error: String(error) });
    try {
      await db.prepare("PRAGMA foreign_keys = ON").run();
    } catch (err) {}
    return false;
...
export async function runMigrations(db, options = {}) {
  ...
  await rebuildShopsTable(db, report);
  ...
  await ensureIndexIfColumns(db, report, "approval_requests", "idx_approval_requests_created", ["created_at"]);
