...
export async function onRequestGet(context) {
  try {
    ...
    const ref = url.searchParams.get("u") || url.searchParams.get("user") || url.searchParams.get("id") || "";
    if (!ref) return jsonResponse({ ok: false, error: "MISSING_USER" }, 400);

    const status = getOnlineStatus(ref, DB_ONLINE_WINDOW_MS);
    const now = Date.now();
    let lastSeenAt = status.lastSeenMs || 0;
    let online = status.online === true;
    let source = "memory";
    if (db && (!lastSeenAt || now - lastSeenAt > DB_ONLINE_WINDOW_MS)) {
      const dbLastSeen = await readLastSeenFromDb(db, ref);
      if (dbLastSeen) {
        ...
        if (dbLastSeen > status.lastSeenMs) {
          recordOnlinePing(ref, { nowMs: dbLastSeen });
        }
      }
    }
    const etag = buildStatusEtag(ref, DB_ONLINE_WINDOW_MS);
