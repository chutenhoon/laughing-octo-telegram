...
async function listImages(db, shopId, limit) {
  ...
  const sql = `
    SELECT id, r2_object_key, position, uploaded_by_role, created_at
      FROM shop_images
     WHERE shop_id = ?
...
export async function onRequestGet(context) {
  ...
  const rows = await listImages(db, shop.id, isAdmin ? null : SELLER_LIMIT);
  const items = [];
...
export async function onRequestPost(context) {
  ...
  if (!isAdmin) {
    ...
    if (current + files.length > SELLER_LIMIT) {
      return jsonResponse({ ok: false, error: "IMAGE_LIMIT", limit: SELLER_LIMIT, current }, 400);
    }
  }
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    if (!isAdmin && file.size > SELLER_MAX_SIZE) {
    ...
    await db
      .prepare(
        `INSERT INTO shop_images (id, shop_id, r2_object_key, position, uploaded_by_role, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`
      )
      .bind(imageId, shop.id, key, position, uploadedByRole, now)
      .run();
